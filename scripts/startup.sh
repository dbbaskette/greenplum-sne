...
# Greenplum Database Startup Script
# Starts SSH daemon and Greenplum database

set -e

echo "=== Greenplum Container Startup ==="
echo "Timestamp: $(date)"
...
if [ ! -f /usr/local/greenplum-db/greenplum_path.sh ]; then
    echo "Greenplum not installed, running installation..."
    /tmp/install-greenplum.sh
else
    echo "Greenplum already installed"
fi
